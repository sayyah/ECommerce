...
{
    public static void UseOnlineUsers(this IApplicationBuilder app, string cookieName = "UserGuid",
        int lastActivityMinutes = 20)
    {
...
public class OnlineUsersMiddleware
{
    ...
    public Task InvokeAsync(HttpContext context, IMemoryCache memoryCache)
    {
        if (context.Request.Cookies.TryGetValue(_cookieName, out var userGuid) == false)
        {
            ...
            context.Response.Cookies.Append(_cookieName, userGuid,
                new CookieOptions { HttpOnly = true, MaxAge = TimeSpan.FromDays(30) });
        }
