...
{
    public class GetBlogsQueryHandler(IUnitOfWork unitOfWork) :
        IQueryHandler<GetBlogsQuery, PagedList<BlogResult>>
    {
        private readonly IBlogRepository _blogRepository = unitOfWork.GetRepository<BlogRepository, Blog>();

        public async Task<PagedList<BlogResult>> HandleAsync(GetBlogsQuery query)
        {
            IQueryable<Blog> entities = _blogRepository.Search(query.PaginationParameters);
