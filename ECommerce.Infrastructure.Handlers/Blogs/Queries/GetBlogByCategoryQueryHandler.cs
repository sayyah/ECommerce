...
{
    public class GetBlogByCategoryQueryHandler(IUnitOfWork unitOfWork) :
        IQueryHandler<GetBlogByCategoryQuery, PagedList<BlogResult>>
    {
        private readonly IBlogRepository _blogRepository = unitOfWork.GetRepository<BlogRepository, Blog>();

        public async Task<PagedList<BlogResult>> HandleAsync(GetBlogByCategoryQuery query)
        {
            IQueryable<Blog> entities = _blogRepository.GetWithInclude(query.CategoryId);
