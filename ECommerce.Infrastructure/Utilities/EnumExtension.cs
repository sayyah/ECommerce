...
public static class EnumExtensions
{
    ...
    public static List<EnumField> GetEnumFields(this Type enumType)
    {
        ...
        {
            var orderAtt =
                enumType.GetField(name).GetCustomAttributes(typeof(OrderAttribute), false)
                    .SingleOrDefault() as OrderAttribute;

            var displayNameAtt =
                enumType.GetField(name).GetCustomAttributes(typeof(NameAttribute), false).SingleOrDefault() as
                    NameAttribute;

            var valueAtt =
                enumType.GetField(name).GetCustomAttributes(typeof(ValueAttribute), false)
                    .SingleOrDefault() as ValueAttribute;
