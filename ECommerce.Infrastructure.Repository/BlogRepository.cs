...
public class BlogRepository(SunflowerECommerceDbContext context) : RepositoryBase<Blog>(context), IBlogRepository
{
    ...
    public IQueryable<Blog> GetByTagText(PaginationParameters paginationParameters)
    {
        return context.Blogs.Where(x => x.Tags.Any(t => t.TagText == paginationParameters.TagText))
                .Include(x => x.Image)
