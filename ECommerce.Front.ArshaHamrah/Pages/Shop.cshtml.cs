...
public class ShopModel : PageModel
{
    ...
    public async Task OnGet(string? path = null, string? search = null, int pageNumber = 1, int pageSize = 9,
        int productSort = 1,
        string? message = null, string? code = null, int minprice = 0, int maxprice = 0, bool isCheckExist = false)
    {
        ...
        if (!string.IsNullOrEmpty(search)) search = $"Name={search}";
        Products = await _productService.TopProducts(categoryId, search, pageNumber, pageSize, productSort, maxprice,
            minprice, IsCheckExist);
    ...
    public async Task<IActionResult> OnPostPriceRange(string? path = null, int minprice = 0, int maxprice = 0,
        bool isCheckExist = false, int productSort = 1)
    {
        ProductSort = productSort;
        IsCheckExist = isCheckExist;
        Min = minprice;
        ...
        Products = await _productService.TopProducts(categoryId, "", 0, 9, productSort, maxprice, minprice,
            IsCheckExist);
        await Initial();
        Products.PaginationDetails.isCheckExist = isCheckExist;
        Products.PaginationDetails.MinPrice = minprice;
